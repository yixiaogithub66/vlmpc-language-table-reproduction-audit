#!/usr/bin/env python3
"""Render the manuscript figures from the public v1.0.4 evidence files.
...
DATA = ROOT / "data"
SUPPLEMENT_DATA = ROOT / "supplement_v8_sanitized" / "data"
...
def plot_distance_distribution() -> None:
    ...
    semantic = read_csv(DATA / "semantic_mpc_authoritative_runs_v8.csv")
    event = read_csv(DATA / "event_requery_controls_20260620_180646.csv")
    event = [row for row in event if row["case"] == "matched_no_event_instruction"]

    groups = [
        ("Unmodified\nbase", [r for r in archived if r["suite"] == "Unmodified controller"], COLORS["red"]),
        ("Unmodified\nabl.", [r for r in archived if r["suite"] == "Unmodified ablation"], COLORS["red"]),
        ("Grounded\nbase", [r for r in archived if r["suite"] == "Grounded controller"], COLORS["blue"]),
        ("Grounded\nabl.", [r for r in archived if r["suite"] == "Grounded ablation"], COLORS["blue"]),
        ("Semantic\nMPC", semantic, COLORS["teal"]),
        ("Event\nmatched", event, COLORS["amber"]),
    ]
    ...
    for index, (label, rows, color) in enumerate(groups):
        sample = values(rows, "final_distance" if rows and "final_distance" in rows[0] else "final_target_world_distance")
    ...
    ax.set_xticks(range(len(groups)), [label for label, _, _ in groups])
    ax.tick_params(axis="x", length=0, pad=3)
    ax.set_yticks([0.00, 0.05, 0.10, 0.20, 0.30])
    ax.set_yticklabels(["0.00", "0.05", "0.10", "0.20", "0.30"])
    fig.subplots_adjust(left=0.14, right=0.985, bottom=0.27, top=0.98)
    save_figure(fig, "final_distance_distribution_v5")


def plot_threshold_matrix() -> None:
    rows = read_csv(DATA / "threshold_sensitivity_v8.csv")
    suites = [
        ("Unmodified controller", 4),
        ("Unmodified ablation", 8),
        ("Grounded controller", 4),
        ("Grounded ablation", 8),
        ("Semantic MPC latest", 18),
    ]
    thresholds = ["0.05", "0.08", "0.10"]
    lookup: dict[tuple[str, str], tuple[str, float]] = {}
    for row in rows:
        suite = row["suite"]
        if suite == "Semantic MPC":
            suite = "Semantic MPC latest"
        if suite in {name for name, _ in suites}:
            lookup[(suite, row["threshold"])] = (row["success"], float(row["success_rate"]))

    fig, ax = plt.subplots(figsize=(3.36, 2.15))
    cmap = LinearSegmentedColormap.from_list("success", ["#EEF2F5", "#A8C6DF", COLORS["teal"]])
    image = np.array([[lookup[(suite, threshold)][1] for threshold in thresholds] for suite, _ in suites])
    ax.imshow(image, cmap=cmap, norm=Normalize(0, 1), aspect="auto", interpolation="nearest")
    for row_index, (suite, n) in enumerate(suites):
        ...
        ax.text(-0.53, row_index, f"{suite}\n(n={n})", ha="right", va="center", fontsize=6.0, color=COLORS["ink"])
    ...
    save_figure(fig, "threshold_sensitivity_v5")


def plot_event_audit() -> None:
    target_cases = ["Full-frame heuristic", "VLM semantic"]
    target_matches = [1, 4]
    event_rows = read_csv(DATA / "event_requery_controls_20260620_180646.csv")
    ...
    audit_alignment(fig, "target_event_audit_v5", require_panel_labels=True)
    save_figure(fig, "target_event_audit_v5", raster=True)
...
def plot_method_pipeline() -> None:
    ...
    bottom_y, bottom_w, bottom_h = 0.13, 0.24, 0.23
    bottom_specs = [
        (0.035, "Unmodified branch", "no state feedback\n0/4 controller | 0/8 ablation", COLORS["light_red"], COLORS["red"]),
        (0.38, "Feedback branch", "privileged state feedback\ngrounded: 23/23 overrides\nsemantic: 18/18 at 0.08", COLORS["light_blue"], COLORS["blue"]),
        (0.725, "Event controls", "matched no-cache controls\ntrigger executes\nno causal recovery gain", COLORS["light_amber"], COLORS["amber"]),
    ]
    ...
    add_arrow(ax, (0.605, top_y - 0.01), (0.155, bottom_y + bottom_h + 0.01))
    add_arrow(ax, (0.635, top_y - 0.01), (0.50, bottom_y + bottom_h + 0.01))
    add_arrow(ax, (0.845, top_y - 0.01), (0.845, bottom_y + bottom_h + 0.01))
    ...
    save_figure(fig, "method_pipeline_v7")
...
def plot_qualitative_frames() -> None:
    ...
    FIGURES.mkdir(parents=True, exist_ok=True)
    fig.savefig(FIGURES / "qualitative_frames_v5.png", dpi=600, bbox_inches=None, pad_inches=0, facecolor="white")
    plt.close(fig)
...
def main() -> None:
    ...
    print("Rendered v5/v7 manuscript figures from authoritative CSVs.")
