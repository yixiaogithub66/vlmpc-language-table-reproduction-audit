...
**Response:** We proofread the full manuscript and synchronized terminology across the text, tables, captions, data filenames, and response letter. The compiled PDF was checked after the v1.0.8 figures and tables were regenerated. The package verifier also rejects empty portable artifacts and credential-like strings.

**Location:** Throughout the manuscript; `scripts/verify_artifact.py`.

### Comment R2.2: Inconsistent success rates

**Response:** We corrected the evaluation denominator and reran the affected import and consistency checks. The authoritative 18-row semantic suite still gives mean 0.064119, sample SD 0.015025, 3/18 selected-target controls at 0.05, and 18/18 selected-target controls at 0.08. The rows are now partitioned by evaluation scope: 15 fixed-target controls (15/15), one natural-language instruction (1/1 requested-to-selected match and instruction-level joint success), one target-image interface row (1/1 selected-target control, not an independent image-grounding test), and one free scene-selection row (1/1 selected-target control). Only the natural-language row contributes to instruction-level metrics, so the instruction denominator is 1, not 17. The independent target-image matrix remains the source for the 8/9 image-grounding result. The fresh raw sweep is separately reported as 0/16 at 0.05, 0/16 at 0.08, and 0/16 at 0.10. Superseded conflicting aggregate files were removed, the threshold table was regenerated, and the verifier recomputes these values.
...
**Response:** We added a bounded factor-isolating audit rather than overclaiming exhaustive causality. The new `root_cause_ablation_v9.csv/.md` separates planner tuning, execution authority, feedback source, and target grounding. The fresh raw sweep remains 0/16, while the archived grounded branch replaces 23/23 raw actions in all 12 diagnostic rows. Under matched fixed-target seeds 45--50 and identical semantic-controller settings, simulator-state feedback succeeds in 6/6 runs whereas the detector/tracker visual-feedback path succeeds in 0/6; the paired final-world-distance gap (visual minus oracle) is $0.254801\pm0.145339$ (sample SD), and all six visual runs are worse. The independent target-image matrix retains 8/9 joint active successes, with the sole failure caused by a yellow-pentagon-to-yellow-star selection error. These comparisons localize the measured boundary to execution-state estimation and target grounding, but they are not an exhaustive causal decomposition; real-robot transfer, broader tasks, and additional factor interactions remain future work.

**Location:** Observed Failure Symptoms and Evidence Boundary (pp. 7--8); `supplement_v9_sanitized/data/revision_20260922/root_cause_ablation_v9.csv`; `root_cause_ablation_v9.md`; `scripts/derive_root_cause_ablation_v9.py`.

### Comment R2.6: Redesign the re-query experiment so its benefit is measurable

**Response:** We corrected the semantic event detector scale and reran the matched controls. The effective semantic settings are a stagnation window of 4 and a minimum world-distance progress of 0.002, recorded in every event row and aggregate; the generic original-planner event parameters are kept separate. With cache disabled, natural re-query fires in 3/3 enabled runs and 0/3 no-event runs, but the paired final distances remain identical seed by seed. This supports trigger execution under the corrected scale, not a causal recovery benefit. Separately, the controlled semantic-fault audit injects a wrong initial target. The CodexCLIClient forced-requery treatment repairs the semantic state in 6/6 and reaches the independent true-target threshold in 5/6, whereas the no-requery control reaches 0/6. This establishes repair under the tested intervention, but it does not satisfy the stronger request to demonstrate a natural failure-conditioned recovery benefit; that causal question remains open and is stated as a limitation.
...
**Response:** We ran a manuscript-wide language, numerical, cross-reference, and artifact audit after regenerating the v1.0.8 figures. We removed superseded aggregate files and an empty event artifact, separated executed-source and public-snapshot hashes, exported the robustness-edge CSV and four portable inputs, corrected stale package paths, corrected the instruction denominator and semantic event threshold units, documented the CodexCLIClient invocation, and verified the authoritative counts. The final integrity check covers the raw 16-run sweep, semantic thresholds and request-selection scope, target-image outcomes, both event-control and six-seed fault-audit tables, nine independent language repeats, visual-feedback result, robustness-edge rows and input paths, source hashes, complete manifest coverage, required figures, and forbidden credential-like strings.
...
The revision now provides a larger and more internally coherent evidence chain, but its scientific conclusion remains deliberately bounded: the unmodified raw VLMPC-style chain is executable yet unsuccessful in the fresh 16-run sweep; privileged feedback stabilizes several diagnostic branches; selected-target semantic control is 18/18 across heterogeneous scopes, but only one natural-language row is instruction-evaluable (1/1), while fixed-target, target-image, and free-scene rows are reported as separate controls; target-image grounding is imperfect at 8/9 in the independent matrix; the CodexCLIClient language audit gives 9/9 target-selection matches and 9/9 joint controls over three instructions and three seeds but remains small; natural event re-query has no demonstrated recovery benefit in the matched controls, while the six-seed forced-fault audit repairs 6/6 semantic states and reaches 5/6 true-target successes versus 0/6 controls; and the first detector/tracker visual-feedback implementation does not close the oracle-to-vision gap. The published v1.0.8 package is internally consistent. Real-robot validation, broader tasks, stronger independent-seed coverage, natural failure-conditioned re-query benefit, and complete causal root isolation remain open.
