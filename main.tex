...
For the live semantic suite, the distance-based control label is computed relative to the selected target recorded for that diagnostic condition. We additionally compare the requested and selected target names: 17/18 rows match, and the corresponding instruction-level joint count is 17/18. Thus the reported 18/18 value is a conditional selected-target control result, not an end-to-end language-instruction success rate.
...
Semantic MPC live & 18 & 18/18* & 17/18 request-selection matches; selected-target control & $0.0641{\pm}0.0150$ & yes \\
...
\end{tabular}
\vspace{2pt}
\parbox{0.97\textwidth}{\footnotesize * The semantic-MPC control count is conditional on the selected target. The portable CSV records 17/18 requested-to-selected target matches and 17/18 instruction-level joint successes; it is not an 18/18 end-to-end instruction result.}
\setlength{\tabcolsep}{6pt}
\end{table*}
...
\begin{figure}[!t]
  ...
  \caption{Threshold sensitivity at 0.05, 0.08, and 0.10, shown as exact selected-target control counts. The outlined column is the project threshold. Fresh raw MPC fails in 0/16 at all three thresholds; semantic MPC changes from 3/18 at 0.05 to 18/18 at 0.08, while requested-to-selected matching is 17/18.}
  \label{fig:threshold}
\end{figure}
...
Semantic MPC live & min 0.030; median 0.064; max 0.0798 & 0.030--0.0798 & 18/18 & 17/18 request-selection matches; selected-target control \\
