...
For the live semantic suite, the distance-based control label is computed relative to the selected target recorded for that diagnostic condition. Of the 18 heterogeneous rows, 17 carry an explicit requested target and are instruction-evaluable; all 17 preserve the requested-to-selected target identity and satisfy the instruction-level joint criterion. The remaining row is a free scene-selection run with no requested object; it is reported as a 1/1 selected-target control and as not applicable (N/A) for instruction metrics. Thus the reported 18/18 value is a conditional selected-target control result, while the instruction-level result is 17/17 among evaluable rows.
...
Semantic MPC live & 18 & 18/18* & 17/17 explicit-target rows match; one scene-selection row N/A for instruction metrics & $0.0641{\pm}0.0150$ & yes \\
...
\end{tabular}
\vspace{2pt}
\parbox{0.97\textwidth}{\footnotesize * The semantic-MPC control count is conditional on the selected target. Among 17 rows with an explicit requested target, the portable CSV records 17/17 requested-to-selected matches and 17/17 instruction-level joint successes. The remaining row is a free scene-selection run with no requested target and is reported separately as a 1/1 selected-target control; it is N/A for instruction metrics.}
\setlength{\tabcolsep}{6pt}
\end{table*}
...
Semantic MPC live & min 0.030; median 0.064; max 0.0798 & 0.030--0.0798 & 18/18 & 17/17 explicit-target matches; one scene-selection row N/A \\
